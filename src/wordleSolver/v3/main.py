...
def give_rank_central(word_list, word_correct, word_wrong_place, word_wrong, splits):
    ...
    for id in range(splits - 1):
        x = multiprocessing.Process(
            target=give_rank,
            args=(
                word_list[split_amount * id : (split_amount * id + 1) - 1],
                word_list,
                word_correct,
                word_wrong_place,
                word_wrong,
...
def give_rank(
    words, word_list, word_correct, word_wrong_place, word_wrong, id, ranks_out
):
    ranks = [None] * len(words)
    threads = []
    for index, word in enumerate(words):
        thread = multiprocessing.Process(
            target=give_one_rank,
            args=(
                word,
                word_list,
                word_correct,
...
def give_one_rank(
    word, word_list, word_correct, word_wrong_place, word_wrong, index, ranks
):
    if len(word) != 5:
        try:
            raise ValueError(
                "Word {} at index {} is shorter than five".format(
                    word, word_list.index(word)
                )
            )
        except ValueError:
            raise ValueError(
                "Word {} is shorter than five, and isn't in word list".format(word)
            )
    word_list_copy, word_correct_copy, word_wrong_copy, word_wrong_place_copy = (
        make_copies(word_list, word_correct, word_wrong_place, word_wrong)
    )
    word_correct_copy, word_wrong_place_copy, word_wrong_copy = parse_input(
        "sssss", word, word_correct_copy, word_wrong_place_copy, word_wrong_copy
    )
    rank = abs(
        len(
            find_all_valid(
...
def find_all_valid(
    words: list, ranks, word_correct: list, word_wrong_place: dict, word_wrong: list
):
    index = 0
