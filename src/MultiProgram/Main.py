...
class Main:
    ...
    def hub(self):
        ...
        while True:
            ...
            if option == "messanger send":
                ...
                show_window("{} is not an option!".format(option), "Hub Error")
            time.sleep(2)
            clear()
    ...
    def original_login(self):
        ...
        if not self.login_check:
            logging(
                "User Failed To Login With Username {} and password {}".format(
                    user, password
                )
            )
    ...
    def other_login(self):
        ...
        if user == self.username and password == self.password:
            ...
            logging(
                "User Failed To Other Login With Username {} and password {}".format(
                    user, password
                )
            )
            clear()
    ...
    def messanger_send(self, message=None, encrypt=None, key=None):
        ...
        self.login_checker()
        if message is None:
            message = question_window(
                "What is the message you would like to send?", "Message Send"
            )
        if encrypt is None:
            while True:
                encrypt = question_window(
                    "Would you like to encrypt the message? (y/n)",
                    "Message Encryption?",
                )
                encrypt = encrypt.lower()
                if encrypt == "y" or encrypt == "n":
                    break
                show_window("That is not y or n" + "Try again", "not y or n")
        if encrypt == "y":
            if key is None:
        ...
        show_window(
            "Your message was saved at message.txt!", "Successfull Message Save"
        )
        time.sleep(2)
        clear()
    ...
    def messager_read(self, encrypt=None, key=None):
        ...
        if encrypt is None:
            while True:
                encrypt = question_window(
                    "Is the message encrypted? (y/n)", "Message Encryption?"
                )
                encrypt = encrypt.lower()
                if encrypt == "y" or encrypt == "n":
                    break
        ...
        show_window("Your message is {}!".format(message), "Message Result")
        clear()

    def clearance_check(self, clearance):
        show_window("To access this program you must log in again!", "Re-Login Needed")
        if not self.other_login():
            show_window("Login Usuccesfull!", "Failed Login")
            return False
    ...
    def view_passwords(self):
        ...
        if not self.clearance_check(100):
            show_window(
                "You don't have the needed clearance level (100), to preform this action",
    ...
    def decrypt_local(self):
        ...
        show_window(
            "Your message is {}".format(pyWrskp.decrypt(key, message)), "Message"
        )
    ...
    def encrypt_local(self):
        ...
        key = number_input_local("What is the key")
        show_window(
            "Your encoded message is:\n" + pyWrskp.encrypt(key, message), "Message"
        )
    ...
    def view_log(self):
        ...
        if not self.clearance_check(50):
            show_window(
                "You don't have the needed clearance level (50), to preform this action",
                "Not enough clearance",
            )
            return
    ...
    def math_game(self):
        ...
        if max_score != []:
            show_window("The current high score is {}".format(max_score), "High Score")
        time.sleep(2)
        countinue = True
        while countinue:
            clear()
            ...
            while True:
                ...
                try:
                    ...
                    show_window("That is not a number, try again", "Not Number")
            if countinue:
                max_score = pyWrskp.read_file("math.txt")
                if user_input == awnser:
                    ...
                    if score == max_score:
                        show_window(
                            "Good Job, you have hit the max score!",
                            "Equal to High Score",
                        )
                    elif score < max_score:
                        show_window(
                            "You still have {} to go before you hit the max score".format(
                                max_score - score
                            ),
                        ...
                        show_window(
                            "You are {} over the current top score".format(
                                score - max_score
                            ),
        ...
        if score > max_score:
            show_window(
                "You beat the current high score by {}".format(score - max_score),
                "Beat High Score",
            )
            pyWrskp.write_file("math.txt", score)
            logging(
                "Player {} beat the max score (of {}) and set a new max score as {}".format(
                    self.username, max_score, score
                )
            )
    ...
    def lowest_common_multiple(self):
        ...
        ma = number_input_local(
            "Max amount of loops (Greater the number, greater the time.)"
        )
        result = check_lowest_common_multiple(numb, numb2, ma)
        if result is False:
            show_window(
                "The program could not find a result in the max of {} you gave.".format(
                    ma
                ),
                "Failed Result",
            )
            ...
            show_window(
                "The lowest common multiple between {} and {} is {}".format(
                    numb, numb2, result
                ),
                "Result",
            )
    ...
    def add_user(self):
        ...
        if not self.clearance_check(100):
            show_window(
                "You don't have high enough clearnance (100)", "Not enough clearance"
            )
            logging(
                "User {} tried and failed, with too little clearance of {} to add a new user".format(
                    self.username, self.security_clearence
                )
            )
            return
        logging("user {} is adding a new user".format(self.username))
        username = question_window("Enter the new user's username", "Username")
        password = question_window("Enter the new user's password", "Password")
        clearance = number_input_local("Enter the new user's clearance")
        name = question_window("What is the new user's name?", "Name")
    ...
    def mastermind(self):
        ...
        show_window(
            "| means correct, and correct place\n"
            + "/ means correct, and wrong place\n"
            + "- means wrong",
            "Rules",
        )
        max_guess = number_input_local(
            "How many guesses do you want to be able to have?"
        )
        guessed = False
        guesses = 0
        while True:
            player_guess_str = question_window(
                "Input your guess use : to seperate numbers\n"
        ...
        if guessed:
            ...
            logging(
                "User won mastermind with {} guesses out of a max of {}".format(
                    guesses, max_guess
                )
            )
        else:
