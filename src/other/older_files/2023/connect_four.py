...
def get_col(player):
    while True:
        col = pyWrskp.number_input(
            "Player {}, which colum would you like to go into?".format(player)
        )
...
def all_checks(x, y, char, x_change, y_change):
    ...
    counter_y = y_change
    while True:
        if ((y + counter_y) <= 5 and (x + counter_x) <= 7) and (
            (y + counter_y) >= 0 and (x + counter_x) >= 1
        ):
