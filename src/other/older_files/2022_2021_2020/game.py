...
class Game:
    def __init__(self, debug=False, health=200, cheat=True):
        ...
        self.health = health
        self.attacks = [
            {"name": "blast", "damage min": 1, "damage max": 20},
            ...
            self.attacks.append(
                {
                    ...
                    "damage min": 999999999999998,
                    "damage max": 999999999999999,
                }
            )
    ...
    def print_info(self):
        ...
        for item in self.attacks:
            ...
            print(
                "\t{} ({} - {} damage)".format(
                    name, item["damage min"], item["damage max"]
                )
            )
        print("Bad Guys:")
    ...
    def choose_bad(self):
        ...
        if num == 1:
            ...
            self.monster(
                "RANDOM KILLER", randint(1, 100), randint(100, 200), randint(100, 200)
            )
        elif self.debug:
            print(
                "debug info: issue, code could not find nums nessary\nsection: self.choose_bad"
            )
        print("\n")
    ...
    def monster(self, name, attack_max, attack_min, monster_health):
        ...
        if self.health <= 0:
            ...
            print("GOOD JOB!")
            print(
                "You killed the {} with {} health to spare!".format(name, self.health)
            )
