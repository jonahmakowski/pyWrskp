...
def getUserName():
    ...
    if us == "":
        raise Exception("Could not access skype!")
    return us
...
def handleFriends(qry):
    ...
    try:
        ...
        return (
            PyFred("ch.xtin.skypingalfred.error", False)
            .addItem(
                "skypeupdate",
                "skype update",
...
def handleMessage(qry):
    ...
    try:
        if ":" in qry and (
            qry.partition(":")[0] in map(lambda s: s[0], _readFriends())
        ):
...
def _skypeError():
    return (
        ...
        .addItem(
            ...
            "skype error",
            "No Skype Access!",
            "Either Skype is not running or you didn't give Alfred access to it, when it asked for it!",
            False,
        )
        .toXML()
    )
...
def _getFriends():
    ...
    path = os.path.expanduser(
        "~/Library/Application Support/Skype/" + getUserName() + "/main.db"
    )
    with contextlib.closing(sqlite3.connect(path).cursor()) as db:
        db.execute(
            "SELECT skypename,fullname,displayname FROM Contacts WHERE type=1 AND is_permanent=1"
        )
        return db.fetchall()
        # Using Skype API:
...
def _getAvatars():
    ...
    with contextlib.closing(sqlite3.connect(path).cursor()) as db:
        for av in db.execute(
            "SELECT skypename,avatar_image FROM Contacts WHERE type=1 AND is_permanent=1"
        ):
            if av[1] is not None:
                with open("avatars/" + av[0] + ".jpeg", "wr") as f:
                    f.write(
                        str(av[1])[1:]
                    )  # For some reason, the first byte is 0, then the jpg starts.
...
def _getAvatar(acc):
    ...
    return (
        ...
        else ("/Applications/Skype.app", "fileicon")
    )
...
def _getLastMessage(acc):
    ...
    with contextlib.closing(sqlite3.connect(path).cursor()) as db:
        db.execute(
            "SELECT timestamp,body_xml FROM Messages WHERE author=? ORDER BY timestamp DESC LIMIT 1",
            (acc,),
        )
        return db.fetchone()
...
def _sendMessageWait(msg):
    ...
    return (
        PyFred("ch.xtin.skypingalfred.wait", False)
        .addItem(
            "skypename_" + m[0],
            msg,
            "Message to: " + m[0],
            _getLastMessageFormated(m[0]),
            True,
            None,
            img,
            imgtype,
...
def _listFriends(fs, sep=": ", lock=True, getLastMessage=False):
    ...
    if len(p._items) == 0:
        p.addItem(
            "skypename_notfound",
            "skype update",
            "No Skype friend found: " + tg,
            "Maybe try updating your friends? Hit enter to do so!",
            True,
        )
    return p.toXML()
...
def _findFriends(tg, sep=": ", lock=True, getLastMessage=False):
    ...
    """
    return _listFriends(
        _matchFriends(tg, _readFriends())[0:5], sep, lock, getLastMessage
    )
...
def _findNewest():
    ...
    with contextlib.closing(sqlite3.connect(path).cursor()) as db:
        db.execute(
            "SELECT skypename,fullname,displayname FROM Contacts WHERE skypename IN "
