...
class Human:
    def __init__(self, fname, lname, age, tec_age=False, _=None):
        ...
        if tec_age == True:
            ...
            tec_age = age
        self.info = {
            "name": {"first name": fname, "last name": lname},
            "age": {
                "str": str(age),
                "int": int(age),
                "tec_age": {"str": str(tec_age), "int": int(tec_age)},
            },
            "login": False,
        }
        ...
        def figure(n_fname, n_lname, year):
            if (
                self.info["name"]["first name"] == n_fname
                and self.info["name"]["last name"] == n_lname
            ) and (self.today.year - year) == self.info["age"]["tec_age"]["int"]:
                return True
            else:
                return False
    ...
    def login(self):
        while True:
            if self.Noah == False and self.Jonah == False:
                ...
                password = input(
                    "What is the password " + self.info["name"]["first name"] + "?\n"
                )
    ...
    def admin_login(self):
        code_name = input(
            "What is your codename " + self.info["name"]["first name"] + "  "
        )
    ...
    def admin(self):
        ...
        report = input(
            "Today's report is that Lola is being very cute, and that she is the best dog ever\n"
        )
        if report == "y":
            self.pin = 1892
            pin = input("what is Jonah's pin?  ")
