...
class ChessBoard:
    ...
    def check(self, color):
        opponent_moves = self.bot_access.find_options_helper(
            self.clone(), "A" if color == "B" else "B", "", check=False
        )
    ...
    def display_board(self):
        ...
        for row in self.board:
            ...
            print("")
        print(
            "Current Score: White: {}, Black: {}".format(
...
class Bot:
    ...
    def find_options_first(self, board: ChessBoard, moves, side):
        ...
        if moves <= 0:
            ...
            for option in cur_options:
                self.new_search_thread(
                    option["newBoard"], moves - 1, "B" if side == "A" else "A", option
                )
                return
    ...
    def find_options_helper(
        self, board: ChessBoard, side, first_move, options=None, check=True
    ):
        if options is None:
            options = []
            ...
            match t:
                case "1":
                    ...
                    if side == "B":
                        ...
                        if (
                            y - 1 in possible and x + 1 in possible
                        ) and board.get_location(x + 1, y - 1) != "00":
                            options = self.append_options(
                                (x, y),
                                (x + 1, y - 1),
                                board,
                ...
                case "2":
                    options = self.check_straights(
                        (x, y), board, options, side, first_move, check
                    )
                case "3":
                    options = self.append_options(
                        (x, y), (x - 1, y + 2), board, options, side, first_move, check
                    )
                    options = self.append_options(
                        (x, y), (x + 1, y + 2), board, options, side, first_move, check
                    )
                    ...
                    options = self.append_options(
                        (x, y), (x - 2, y + 1), board, options, side, first_move, check
                    )
                    options = self.append_options(
                        (x, y), (x + 2, y + 1), board, options, side, first_move, check
                    )
                ...
                case "6":
                    options = self.append_options(
                        (x, y), (x + 1, y), board, options, side, first_move, check
                    )
                    options = self.append_options(
                        (x, y), (x + 1, y + 1), board, options, side, first_move, check
                    )
                    options = self.append_options(
                        (x, y), (x, y + 1), board, options, side, first_move, check
                    )
                    options = self.append_options(
                        (x, y), (x, y - 1), board, options, side, first_move, check
                    )
    ...
    @staticmethod
    def append_options(
        xy, xy2, board: ChessBoard, options: list, side: str, first_move, check
    ):
    ...
    def check_diagonals(self, xy, board, options, side, first_move, check):
        x, y = xy
        options = self.direction_check(
            (x, y), (1, 1), board, options, side, first_move, check
        )
        options = self.direction_check(
            (x, y), (-1, 1), board, options, side, first_move, check
        )
    ...
    def check_straights(self, xy, board, options, side, first_move, check):
        x, y = xy
        options = self.direction_check(
            (x, y), (1, 0), board, options, side, first_move, check
        )
        options = self.direction_check(
            (x, y), (-1, 0), board, options, side, first_move, check
        )
        options = self.direction_check(
            (x, y), (0, 1), board, options, side, first_move, check
        )
        options = self.direction_check(
            (x, y), (0, -1), board, options, side, first_move, check
        )
    ...
    def direction_check(self, xy, direction, board, options, side, first_move, check):
        ...
        while True:
            if x + x_counter > 7 or y + y_counter < 0:
                ...
                break
            elif board.get_location(
                x + x_counter, y + y_counter
            ) == "00" or side not in board.get_location(x + x_counter, y + y_counter):
                self.append_options(
                    (x, y),
                    (x + x_counter, y + y_counter),
                    board,
