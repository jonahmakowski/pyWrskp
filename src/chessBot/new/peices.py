class ChessPiece:
    ...
    def debug(self):
        return (
            "White {} at {}, {}".format(self.name, self.position_x, self.position_y)
            if self.side == 0
            else "Black {} at {}, {}".format(
    ...
    def list_moves(self, board):
        ...
        for move in self.moves:
            if not (
                (self.position_x + move[0] < 0 or self.position_y + move[1] < 0)
                or (self.position_x + move[0] > 7 or self.position_y + move[1] > 7)
            ):
                if (
                    board.get_location(
                        self.position_x + move[0], self.position_y + move[1]
                    )
                    is None
                ):
                    legal_moves.append(
    ...
    def check_range(self, start: int, end: int, x_change: int, y_change: int, b):
        ...
        for distance in range(start, end, direction):
            if self.position_x + cur_x > 7 or self.position_y + cur_y > 7:
                ...
                b.get_location(self.position_x + cur_x, self.position_y + cur_y) is None
            ):
                ...
                b.get_location(self.position_x + cur_x, self.position_y + cur_y)
                is not None
            ):
                if (
                    b.get_location(
                        self.position_x + cur_x, self.position_y + cur_y
                    ).side
                    != self.side
                ):
...
class Pawn(ChessPiece):
    ...
    def list_moves(self, board):
        ...
        if (self.side == 0 and b[self.position_y + 2][self.position_x] is None) and b[
            self.position_y + 1
        ][self.position_x] is None:
            legal_moves.append((self.position_x, self.position_y + 2))
        elif (self.side == 1 and b[self.position_y - 2][self.position_x] is None) and b[
            self.position_y - 1
        ][self.position_x] is None:
            legal_moves.append((self.position_x, self.position_y - 2))
        ...
        if 0 < self.position_x + 1 < 8 and 0 < self.position_y + 1 < 8:
            if self.side == 0 and (
                b[self.position_y + 1][self.position_x + 1] is not None
