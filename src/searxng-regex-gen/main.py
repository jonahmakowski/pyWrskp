...
def generate_website_regex(website_input):
    ...
    domain = (
        parsed_url.netloc
    )  # This will be 'www.example.com', 'example.com', 'sub.example.co.uk', or empty
...
if __name__ == "__main__":
    ...
    while True:
        user_input = input(
            "\nEnter a website URL or domain (e.g., 'google.com', 'https://example.org/path'): "
        ).strip()
        if not user_input:
        ...
        if generated_regex.startswith("Error:"):
            ...
            try:
                ...
                example_domain_base = (
                    parsed_for_example.netloc
                    if parsed_for_example.netloc
                    else user_input
                )
