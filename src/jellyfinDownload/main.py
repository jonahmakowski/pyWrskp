...
def search_movie(title, year=None):
    ...
    params = {
        "q": (
            ...
            if year is None
            else f'title:"{title}" AND mediatype:movies AND year:{year}'
        ),
...
def download_movie(identifier, dest_folder):
    ...
    for file in metadata.get("files", []):
        if (file["name"].endswith((".mkv", ".avi")) and not mp4) or (
            ...
            with requests.get(file_url, stream=True) as r:
                ...
                with tqdm(
                    total=total_size, unit="B", unit_scale=True, desc=file["name"]
                ) as progress_bar:
...
def rename_and_move(file_path, movie_title, year=None):
    ...
    # Format new filename
    new_filename = (
        f"{formatted_title} ({year}){imdb_tag}{os.path.splitext(file_path)[-1]}"
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        for index, movie in enumerate(results):
            print(
                f"Index: {index}, Title: {movie['title']}, Year: {movie.get('year', 'Unknown')}, Creator: {movie.get('creator', 'Unknown')}, URL: https://archive.org/details/{movie['identifier']}"
            )
