...
blogPosts_list = [
    ...
    {
        "creator": {
            "username": "Mr. Payne's 4/5 class 2020-21",
            "user": "Mr.Payne's+45+class+2020-21",
        },
...
def post(creator):
    ...
    if not usr:
        ...
        return render_template(
            "Blog_outline.html",
            title=post["creator"]["username"] + "'s blog post",
            post=post,
        )
...
def ceo_info():
    if request.method == "POST":
        ...
        name = "Name:         " + CEO["name"]
        email = "Emails:       {}, {}, {}".format(
            CEO["emails"]["1"], CEO["emails"]["2"], CEO["emails"]["3"]
        )
        link = "Youtube channel: " + CEO["link"]
        print(name + "\n" + email + "\n" + link)
        return render_template(
            "CEO_info.html", title="CEO info", name=name, email=email, link=link
        )
    else:
...
def page():
    return render_template(
        "page.html", title="List of Pages | This is a list of pages on this website"
    )
...
def passwords():
    if request.method == "POST":
        ...
        return render_template(
            "password_show.html", title="Passwords", password=password
        )
    elif request.method == "GET":
...
def code():
    if request.method == "POST":
        ...
        return render_template(
            "coder_show.html", title="coder decoder show", message=message, key=key
        )
    elif request.method == "GET":
...
def schedule_create():
    if request.method == "POST":
        ...
        start_time = "{}:{}".format(
            request.form.get("start hr"), request.form.get("start min")
        )
        end_time = "{}:{}".format(
            request.form.get("end hr"), request.form.get("end min")
        )
...
def noah_info():
    emails = [
        {"email_name": ".kmjn", "email": "noah.kmjn@gmail.com"},
        {"email_name": "@makowski.ca", "email": "noah@makowski.ca"},
        {"email_name": "@makowski.at", "email": "noah@makowski.at"},
    ]
    return render_template(
        "noah_info.html", title="Noah's Info", name="Noah Makowski", emails=emails
    )
