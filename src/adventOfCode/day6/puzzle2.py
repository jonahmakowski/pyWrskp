...
def mainloop(data: list, direc: tuple, was: any):
    ...
    x, y = find_guard(data)
    dirx, diry = direc
    if (data[y+diry][x+dirx] == "." or data[y+diry][x+dirx] == "X") or isinstance(data[y+diry][x+dirx], int):
...
def main():
    ...
    for index, option in enumerate(options):
        threads.append(multiprocessing.Process(target=check_valid,
                                               args=(option, valid_options, index)))
        threads[-1].start()
        if index % 40 == 0:
            for thread in threads:
                ...
                print('Done')
            threads = []
            print('So far {} valid options. Started threads up to {} of {}'.format(count(valid_options),
                                                                                   index,
                                                                                   len(options)))
