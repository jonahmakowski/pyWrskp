...
fn board_location() {
    ...
    assert_eq!(new_board.at_location(&types::Position { x: 3, y: 3 }), None);
}
...
fn pawn_move_forward_one() {
    ...
    assert!(
        board
            .move_piece_with_error_checking(
...
fn rook_can_capture() {
    ...
    board
        .pieces
        .retain(|p| p.get_position() != types::Position { x: 0, y: 1 });
...
fn queen_can_capture() {
    ...
    board
        .pieces
        .retain(|p| p.get_position() != types::Position { x: 3, y: 1 });
...
fn king_move_one_square() {
    ...
    board.pieces.retain(|p| {
        let pos = p.get_position();
        pos != types::Position { x: 3, y: 1 } && pos != types::Position { x: 4, y: 1 }
    });
...
fn king_cannot_move_two_squares() {
    ...
    board
        .pieces
        .retain(|p| p.get_type() == types::PieceType::KING);
...
fn does_location_match_color() {
    ...
    // White piece at (0, 0)
    assert_eq!(
        board.does_location_match_color(&types::Position { x: 0, y: 0 }, &types::Side::WHITE),
        Some(true)
    );
    assert_eq!(
        board.does_location_match_color(&types::Position { x: 0, y: 0 }, &types::Side::BLACK),
        Some(false)
    );

    // Black piece at (0, 7)
    assert_eq!(
        board.does_location_match_color(&types::Position { x: 0, y: 7 }, &types::Side::BLACK),
        Some(true)
    );
    assert_eq!(
        board.does_location_match_color(&types::Position { x: 0, y: 7 }, &types::Side::WHITE),
        Some(false)
    );

    // Empty square
    assert_eq!(
        board.does_location_match_color(&types::Position { x: 4, y: 4 }, &types::Side::WHITE),
        None
    );
}
...
fn invalid_move_keeps_turn_and_position() {
    ...
    assert_eq!(board.turn, types::Side::WHITE);
    assert_eq!(
        board.at_location(&types::Position { x: 0, y: 1 }),
        original_piece
    );
}
...
fn piece_symbols_correct() {
    ...
    assert_eq!(
        board
            .at_location(&types::Position { x: 0, y: 0 })
            .unwrap()
            .get_symbol(),
        '♖'
    );
    assert_eq!(
        board
            .at_location(&types::Position { x: 1, y: 0 })
            .unwrap()
            .get_symbol(),
        '♘'
    );
    assert_eq!(
        board
            .at_location(&types::Position { x: 2, y: 0 })
            .unwrap()
            .get_symbol(),
        '♗'
    );
    assert_eq!(
        board
            .at_location(&types::Position { x: 3, y: 0 })
            .unwrap()
            .get_symbol(),
        '♕'
    );
    assert_eq!(
        board
            .at_location(&types::Position { x: 4, y: 0 })
            .unwrap()
            .get_symbol(),
        '♔'
    );
    assert_eq!(
        board
            .at_location(&types::Position { x: 0, y: 1 })
            .unwrap()
            .get_symbol(),
        '♙'
    );
    ...
    assert_eq!(
        board
            .at_location(&types::Position { x: 0, y: 7 })
            .unwrap()
            .get_symbol(),
        '♜'
    );
    assert_eq!(
        board
            .at_location(&types::Position { x: 1, y: 7 })
            .unwrap()
            .get_symbol(),
        '♞'
    );
    assert_eq!(
        board
            .at_location(&types::Position { x: 2, y: 7 })
            .unwrap()
            .get_symbol(),
        '♝'
    );
    assert_eq!(
        board
            .at_location(&types::Position { x: 3, y: 7 })
            .unwrap()
            .get_symbol(),
        '♛'
    );
    assert_eq!(
        board
            .at_location(&types::Position { x: 4, y: 7 })
            .unwrap()
            .get_symbol(),
        '♚'
    );
    assert_eq!(
        board
            .at_location(&types::Position { x: 0, y: 6 })
            .unwrap()
            .get_symbol(),
        '♟'
    );
...
fn index_and_empty_update_after_capture() {
    ...
    assert_eq!(
        board
            .index_at_location(&types::Position { x: 3, y: 4 })
            .is_some(),
        true
    );
...
fn king_under_attack_by_knight() {
    let pieces = vec![
        ...
        types::ChessPiece::new(
            ...
            types::Side::BLACK,
            types::Position { x: 2, y: 3 },
        ),
    ];

    let board = types::Board {
        pieces,
        turn: types::Side::WHITE,
    };
...
fn checkmate_when_king_boxed_and_attacked() {
    ...
    let blockers = [
        types::Position { x: 3, y: 3 },
        types::Position { x: 3, y: 4 },
        types::Position { x: 3, y: 5 },
        types::Position { x: 4, y: 3 },
        /* king */ types::Position { x: 4, y: 5 },
        types::Position { x: 5, y: 3 },
        types::Position { x: 5, y: 4 },
        types::Position { x: 5, y: 5 },
    ];

    for pos in blockers.iter() {
        pieces.push(types::ChessPiece::new(
            types::PieceType::PAWN,
            types::Side::WHITE,
            *pos,
        ));
