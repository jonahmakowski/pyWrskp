...
class Weather:
    ...
    def getForecasthourly(self):
        geocode = requests.get(
            "https://geocoding.geo.census.gov/geocoder/locations/address?street={}&city={}&state={}&zip={}&benchmark=4&format=json".format(
                self.street, self.city, self.state, self.zipcode
            )
        )
        # print(geocode.text)
        coordinates = geocode.json()["result"]["addressMatches"][0]["coordinates"]
        gridpoints = requests.get(
...
def plt_display_forecast(item, hour):
    if hour == "d":
        for lis in item:
            ...
            plt.plot(
                lis.timesNight, lis.tempsNight, "o-", label=lis.loc_name + " night"
            )
...
dc_zipcode = "20500"
other_street = (
    input("What is the number of your loc   ")
    + "+"
    + input("What is the road name   ")
    + "+"
    + input("What is the dr, ave, rd, etc   ")
)
...
NY_near = Weather(
    ny_near_street, ny_near_city, ny_near_state, ny_near_zipcode, "NY Near", hour
)
DC = Weather(dc_street, dc_city, dc_state, dc_zipcode, "DC", hour)
NY = Weather(ny_street, ny_city, ny_state, ny_zipcode, "NY", hour)
try:
    other = Weather(
        other_street, other_city, other_state, other_zipcode, other_name, hour
    )
...
if chart == "y":
    while True:
        loc = input(
            "What loc would you like to print? (all/DC/NY Near/NY, or if it worked, "
            + other_name
            + " if it worked)   "
        )
        if loc == "all":
