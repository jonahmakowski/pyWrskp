...
def menu():  # display rectangles with pen-colors and buttons to change the radius
    ...
    pygame.draw.circle(
        screen,
        currentColour,
        (DIS_WIDTH - int(1.5 * max_radius), int(recHeight / 2)),
        radius,
    )
...
while keep_going:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            ):  # change the canvas background to the current color
            ...
            ):  # increase the radius size (same as through the + button)
                radius += radius_change
                if radius > max_radius:
                    radius = max_radius
            elif (
                event.key == pygame.K_d
            ):  # decrease the radius size (same as through the - button)
                radius -= radius_change
                if radius < min_radius:
                    radius = min_radius
    ...
    if mouse_down:  # mouse not pointing to menu
        ...
        if spot[1] > recHeight:  # mouse in the painting area
            ...
            if margin < radius:  # paint only below the menu bar
                ...
                paint_radius = radius
            if eraser != currentColour:
                pygame.draw.circle(screen, currentColour, spot, paint_radius)
            elif eraser == currentColour:
                pygame.draw.circle(screen, currentColour, spot, paint_radius * 2)
            pygame.display.update()
