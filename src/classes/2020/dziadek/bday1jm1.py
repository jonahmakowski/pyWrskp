...
while True:
    ...
    if time_to_birthday_2 * days_in != time_to_birthday:
        time_to_birthday_2 = (
            str(time_to_birthday_2)
            + " months and "
            + str(int(time_to_birthday.days) - (time_to_birthday_2 * days_in))
            + " days"
        )
    ...
    setup.write(
        "please type in y/n for the following question (you must do this in the shell) "
        + continue_q
    )
