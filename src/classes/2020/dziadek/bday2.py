...
class NextBday:
    def __init__(self, b_month, b_day, b_yr=0, cheat_today=False):
        ...
        try:
            ...
            print(
                "Month number must be in [1, 12]. Provided value of "
                + str(b_month)
                + " is illegal."
            )
        ...
        try:
            ...
            print(
                "Day number must be in [1, 31]. Provided value of "
                + str(b_day)
                + " is illegal."
            )
            # sys.exit('wrong day')
        ...
        if (
            self.next_bday < self.today
        ):  # already passed this year, consider bday next year
    ...
    def bday_info(self):
        ...
        if self.bday_defined:
            print(
                ...
                + str(self.next_bday)
                + " you will be "
                + str(self.next_bday.year - self.birth.year)
                + " years old."
            )
        else:
    ...
    def bday_in(
        self,
    ):  # calculate and print number of months and days until next birthday
    ...
    def bday_in2(self):
        ...
        print(
            "Number of months and days until " + str(self.next_bday) + " not available."
        )
...
bday = NextBday(
    5, 29, 2010, True
)  # ctor for testing (uses the Today value specified in the ctor)
bday.bday_info()
bday.bday_in()
pass
