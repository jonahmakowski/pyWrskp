...
while True:
    while True:
        command = input(
            "What command would you like to use? (forward, backward, left, or right)\n"
        )
        command = command.lower()
        if (command == "forward" or command == "backward") or (
    ...
    while True:
        distance = input(
            "What number would you like to use, for example 90 degrees right, or 100 units forward!\n"
        )
        if distance.isdigit():
