...
def send_message(message, username, other_user):
    try:
        data = {"message": message, "user_sent": other_user.lower()}
...
def home():
    ...
    return render_template(
        "home.html", username=session["username"], sidebar_options=user_pages(users)
    )
...
def chat(user):
    ...
    if request.method == "POST":
        if user != "Ai":
            ...
            send_message(
                message_dic_to_text(chats, user, session["username"]),
                user,
                session["username"],
            )
        else:
            ...
            new_message = ollama.chat(model="llama3.2", messages=chats_ai)["message"][
                "content"
            ]
            new_message.replace("\t", "    ")
            chats = add_to_chat_dic(
                load_chats_from_file(), "Ai", session["username"], new_message
            )
            save_chats_to_file(chats)
            send_message(
                message_dic_to_text(chats, user, session["username"]),
...
def admin_page():
    if (
        "username" not in session.keys()
        or not get_user(session["username"], users)["admin"]
    ):
        return redirect(url_for(login))
    else:
        if request.method == "POST":
            ...
            return render_template(
                "admin.html", env=cur_env, usernames=cur_usernames, message="Saved"
            )
        else:
            cur_env = load_file(".env")
            cur_usernames = load_file("usernames.txt")
