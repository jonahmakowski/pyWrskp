            ...
            users.append(
                {
                    "user": username,
                    "password": os.getenv(username),
                    "admin": True if index == 0 else False,
                }
            )
        else:
            print(
                "User {} does not have a defined password. Terminating".format(username)
            )
            exit()
...
def add_to_chat_dic(ls, sender, respondent, message):
    ...
    if not found:
        ls.append(
            {
                "usernames": [sender, respondent],
                "chat": [{"username": sender, "message": message}],
            }
        )
...
def convert_to_ai_message_system(ls, user):
    ...
    new_chat = [
        {
            "role": "system",
            "content": "You are an AI assistant running on a local chat platform. The other person is called {}. You can use markdown bolding and italics. You are the LLM llama3.2.".format(
                user
            ),
        }
    ]
